...
export default function CreateGroupModal({ onClose, onCreated }) {
  ...
  const handleSearch = async (q) => {
    ...
    setSearching(true);
    try {
      const res = await api.get(`/v1/search/users?query=${q}`);
  ...
  const handleCreate = async () => {
    if (!groupName.trim()) { toast.error("Nhập tên nhóm!"); return; }
    if (selectedUsers.length < 1) { toast.error("Chọn ít nhất 1 thành viên!"); return; }

    setLoading(true);
    try {
      await groupApi.createGroup(groupName.trim(), selectedUsers.map(u => u.username));
      ...
      <div className="bg-[var(--card)] rounded-2xl w-full max-w-md mx-4 shadow-xl">
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedUsers.map(u => (
                <div key={u.username}
                  ...
                  <button onClick={() => toggleUser(u)} className="ml-1 hover:text-red-600">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ))}
          ...
          <div className="max-h-48 overflow-y-auto space-y-1">
            {searching && (
              <p className="text-xs text-center text-[var(--muted-foreground)] py-2">Đang tìm...</p>
            )}
            {searchResults.map(user => {
              const selected = selectedUsers.find(u => u.username === user.username);
              return (
                <div
                  key={user.username}
        ...
        <div className="flex gap-2 p-4 border-t border-[var(--border)]">
          ...
          >
            {loading ? "Đang tạo..." : `Tạo nhóm (${selectedUsers.length + 1} người)`}
          </button>
        </div>
      </div>
    </div>
