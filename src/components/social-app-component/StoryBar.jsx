  ...
  return (
    <div className="flex-shrink-0 w-[110px]">
      <div className="relative w-[110px] h-[190px] rounded-2xl overflow-hidden bg-[var(--card)] border border-[var(--border)]">
        {/* Background media nếu đã có story — click để xem */}
        <div
          className="absolute inset-0 cursor-pointer group"
          onClick={hasMyStory ? onOpenMyStory : onOpenCreate}
        >
          {hasMyStory && firstStory?.mediaUrl ? (
            ...
            <div className="w-full h-full bg-gradient-to-b from-blue-400 to-purple-500 opacity-20" />
          )}
        </div>
        ...
        {/* Nút + luôn mở modal tạo mới — tách khỏi click xem story */}
        <button
          onClick={(e) => {
            e.stopPropagation();
            onOpenCreate();
          }}
          className="absolute bottom-8 left-1/2 -translate-x-1/2 w-9 h-9 rounded-full bg-blue-500 border-4 border-[var(--background)] flex items-center justify-center shadow-lg hover:scale-110 transition-transform z-10"
          title="Tạo story mới"
        >
        ...
        {/* Label */}
        <p className="absolute bottom-2 left-0 right-0 text-center text-[11px] font-semibold text-white leading-tight px-1 pointer-events-none">
          {hasMyStory ? "Story của bạn" : "Tạo story"}
        </p>

        {/* Count badge */}
        {hasMyStory && (
          <div className="absolute top-2 right-2 bg-blue-500 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center pointer-events-none">
            {myStories.length}
          </div>
        )}
      </div>
