...
public class GetFeedUseCase {
    ...
    public GetFeedUseCase(PostRepository postRepository, FileStorage fileStorage) {
    ...
    public List<PostResponse> execute(String userId, int skip, int limit) {
        return postRepository.findFeedByUserId(userId, skip, limit)
                .stream()
                .map(post -> toResponse(post, postRepository.isLikedByUser(userId, post.getId())))
                .toList();
    }
