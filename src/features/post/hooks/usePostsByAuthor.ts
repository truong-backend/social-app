...
const getPostsByAuthorApi = async (
  ...
  const response = await axiosInstance.get(`/api/posts/author/${authorId}`, {
    params: { viewerId, skip, limit },
  })
  return unwrapData(response) ?? []
}

export const usePostsByAuthor = (authorId: string, viewerId: string) => {
  return useInfiniteQuery({
    queryKey: POST_QUERY_KEYS.byAuthor(authorId),
    queryFn: ({ pageParam }) =>
      getPostsByAuthorApi(authorId, viewerId, pageParam as number, FEED_PAGE_SIZE),
    initialPageParam: 0,
