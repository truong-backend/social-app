...
import type {
  StringeeClientInstance,
  StringeeCallInstance,
} from "../types/stringee.types";
...
export const StringeeSingleton = {
  getClient: () => _client,
  setClient: (c: StringeeClientInstance | null) => {
    _client = c;
  },

  getCall: () => _call,
  setCall: (c: StringeeCallInstance | null) => {
    _call = c;
  },
  ...
  setLocalVideoEl: (el: HTMLVideoElement | null) => {
    ...
    if (el && _pendingLocalStream) {
      ...
      el.play().catch(() => {}); // ← thêm .play() explicit
