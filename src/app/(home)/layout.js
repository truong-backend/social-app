...
function GlobalCallInterface() {
    ...
    // ✅ FIX: Tính trực tiếp, KHÔNG dùng useState + useEffect
    // useState tạo ra 1 render cycle delay → CallVideo mount trễ
    // → video DOM refs chưa được đăng ký khi addremotestream fire
    const showCallVideo = !!(currentCall || isCallEnding);

    return (
        <>
            {/* Popup khi có cuộc gọi đến, chưa pick up */}
            ...
            {/* CallVideo — mount ngay khi currentCall set, không delay */}
            {showCallVideo && <CallVideo onCallEnd={() => {}} />}
        </>
    );
}
        ...
        >
            <div className="px-3 py-5 mb-1">
                ...
                <a href="/home" className="flex items-center justify-center lg:hidden">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-7 h-7 text-[var(--foreground)]">
...
function MainLayoutContent({ children }) {
    ...
    useEffect(() => {
        const handleNewMessage = (e) => {
            if (pathname === "/chats" && !activeChatId) {
                ...
                setActiveTargetUser(e.detail.sender);
            }
        };
        const handleOpenChat = (e) => {
            setActiveChatId(e.detail.chatId);
            setActiveTargetUser(e.detail.targetUser);
        };
    ...
    useEffect(() => { if (mounted) prevThemeRef.current = resolvedTheme; }, [resolvedTheme, mounted]);
                    ...
                    <div className="flex flex-col w-full h-full pt-4 pr-4 gap-4">
                        {activeChatId && activeTargetUser ? (
                            <Chatbox
                                ...
                                onBack={() => { setActiveChatId(null); setActiveTargetUser(null); }}
                                onChatCreated={(id, user) => { setActiveChatId(id); setActiveTargetUser(user); }}
                                beToken={token}
                                recipientId={activeTargetUser?.id || activeTargetUser?.userId}
                            />
                        ) : (
                            <ChatList
                                onSelectChat={(chatId, user) => { setActiveChatId(chatId); setActiveTargetUser(user); }}
        ...
        <>
            ...
            {!isInCall && (
                <div className="h-screen flex flex-col">
                    ...
                    <div className="flex flex-1 bg-[var(--background)] text-[var(--foreground)] transition-colors duration-500 overflow-hidden" style={{ height: "100vh" }}>
                        <LeftSidebar narrow={hideRightSidebar} />
                        <main className="flex-1 h-screen overflow-y-auto" style={{ paddingTop: pathname === "/chats" ? "0" : mainPaddingTop }}>
            ...
            {isInCall && (
                <div className="h-screen flex flex-col">
                    <div className="flex flex-1 bg-[var(--background)] text-[var(--foreground)] overflow-hidden" style={{ height: "100vh" }}>
    ...
    return shouldAnimate ? (
        <AnimatePresence mode="wait">
            <MotionContainer key={resolvedTheme} modeKey={resolvedTheme} effect="fadeUp" duration={0.25}>
                {layoutContent}
            </MotionContainer>
        </AnimatePresence>
    ) : layoutContent;
